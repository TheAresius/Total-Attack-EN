...
const TA_LABELS = {
    ...
    "back_attack_dmg": "Back Attack Damage (%)",
    "hp_recovery": "HP Recovery (%)", 
    "mp_recovery": "MP / AP Recovery (%)",
    
    "all_skill_dmg": "All Skill Damage (%)",
    "boss_dmg": "Boss Damage (%)",
    "polarize": "Attack / Attacked Damage (%)",
    "normal_skill_dmg": "Normal Skill Damage (%)",
    "awk_skill_dmg": "Awakening Skill Damage (%)",
    "mp1_dmg": "MP1 Damage (%)",
    "mp2_dmg": "MP2 Damage (%)",
    "mp3_dmg": "MP3 Damage (%)",
    "mp4_dmg": "MP4 Damage (%)",

    "mp_cost": "MP Cost Reduction (%)",
    "mp1_cost": "MP1 Cost Reduction (%)",
    "mp2_cost": "MP2 Cost Reduction (%)",
    "mp3_cost": "MP3 Cost Reduction (%)",
    "mp4_cost": "MP4 Cost Reduction (%)",
    "general_cd": "Cooldown Reduction (%)",
    "mp1_cd": "MP1 Cooldown Reduction (%)",
    "mp2_cd": "MP2 Cooldown Reduction (%)",
    "mp3_cd": "MP3 Cooldown Reduction (%)",
    "mp4_cd": "MP4 Cooldown Reduction (%)",

    "hell_spear_chance": "Hell Spear Chance (%)",
    "hell_spear_dmg": "Hell Spear Damage",
    "crit_dmg_red": "Critical Damage Reduction (%)",
};
...
function saveCurrentTA() {
    ...
    if (!resultSpan || resultSpan.innerText === "-" || resultSpan.innerText === "0") {
        showAlert("Attention", "Calculate a valid result before saving!", "warning");
        return;
    }
          ...
          <div class="response-list-item">
            <div class="response-list-header">
                ...
                <div class="action-buttons">
                    <button class="icon-btn fav-btn" onclick="toggleFavorite(${item.id})" title="Favorite">
                    ...
                    <button class="icon-btn load-btn" onclick="loadHistoryItem(${item.id})" title="Load Result">
                    ...
                    <button class="icon-btn delete-btn" onclick="deleteHistoryItem(${item.id})" title="Remove Result">
                        <svg viewBox="0 0 24 24" width="18" height="18" stroke="currentColor" stroke-width="2" fill="none" stroke-linecap="round" stroke-linejoin="round"><polyline points="3 6 5 6 21 6"></polyline><path d="M19 6v14a2 2 0 0 1-2 2H7a2 2 0 0 1-2-2V6m3 0V4a2 2 0 0 1 2-2h4a2 2 0 0 1 2 2v2"></path><line x1="10" y1="11" x2="10" y2="17"></line><line x1="14" y1="11" x2="14" y2="17"></line></svg>
                    </button>
                    <button class="icon-btn collapse-btn" onclick="toggleItemCollapse(${item.id})" title="Expand/Colapse">
                ...
                <div class="history-anim-inner">
                    <div class="response-list-item-content">
                        ...
                        <div class="stat-group">
                            <div class="response-list-item-title" style="color:#FFB347; font-weight:bold; border-bottom:1px solid rgba(255,255,255,0.1); margin-bottom:5px;">Base Stats</div>
                            <div class="response-list-item-value">Character Type: <span class='history-stat-value'>${item.inputs.character_type}</span></div>
                            ${getStatLine('attack')}
                            ${getStatLine('defense')}
                            ${getStatLine('hp')}
                        ...
                        <div class="stat-group">
                            <div class="response-list-item-title" style="color:#2EFFA8; font-weight:bold; border-bottom:1px solid rgba(255,255,255,0.1); margin-bottom:5px;">Support</div>
                            ${getStatLine('mp_cost')}
                    ...
                    <div class="stat-group result-group" style="margin-top: 15px;">
                        <div class="response-list-item-TA" style="font-size:1.2rem;">Total Attack: <span class="yellow">${item.totalAttack}</span></div>
                    </div>
                </div>
            </div>
          </div>
...
function toggleHistorySelection(id, checkbox) {
    if (checkbox.checked) {
        if (selectedHistoryIds.size >= 2) {
            ...
            showAlert('Limit', 'You can only select two to compare.', 'warning');
            return;
        }
...
function compareSelected() {
    ...
    modalText.innerHTML = `
        <div class="modal-header">
            <div class="modal-name">Total Attack Compare</div>
        </div>
        <div class="modal-body">
            <table class="compare-table" style="table-layout: fixed; width: 100%;">
                ...
                <tbody>
                    <tr>
                        <td class="compare-row-label">Total Attack</td>
                        <td>${rowTA.htmlA}</td>
                        <td>${rowTA.htmlB}</td>
                    </tr>
                    <tr>
...
function clearHistory() {
    askConfirm(
        "Clean history?", 
        "All of your saved result (except favorites) will be lost. Continue?", 
        () => {
