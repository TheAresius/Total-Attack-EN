function atk_total() {
  ...
  if (check_error(ATK, DEF, HP, sATK, sDEF,crit_r, crit_d, rec_HP, rec_MP)){
    result.innerHTML = `<div>${"Total Attack: "} <span class="yellow">${final_result} ± ${est_error}</span>`;
    
  } else {
    result.innerHTML =`<div> ${"Total Attack: "}<span class = "yellow">${'-'}</span>`;
    result.innerHTML +=`<div> <span class = "error">Check the entered values.</span>`
  }
}
